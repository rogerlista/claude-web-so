...
import { createAddSalePaymentUseCase } from './add-sale-payment'

describe('AddSalePayment Use Case', () => {
  it('should add payment to a sale', async () => {
    // Setup
    ...
    // Execute
    const useCase = createAddSalePaymentUseCase(mockRepository)
    const result = await useCase({
      saleId: 'sale-123',
      paymentMethodCode: '01', // Dinheiro
      amount: 50.0,
    })
    ...
    if (result.ok) {
      ...
      expect(result.value.payments[0]?.paymentMethod.code).toBe('01')
      expect(result.value.payments[0]?.amount).toBe(50.0)
      // Calculate paid amount from payments
      const paidAmount = result.value.payments.reduce((sum, p) => sum + p.amount, 0)
      expect(paidAmount).toBe(50.0)
    }
  })
  ...
  it('should fail for invalid payment method code', async () => {
    const saleIdResult = createSaleId('sale-123')
    const customerIdResult = createCustomerId('customer-456')
