...
import type { InventoryRepository, RepositoryError } from '../ports/inventory-repository'
import {
...
describe('RegisterStockMovement Use Case', () => {
  it('should register entrada movement with valid data', async () => {
    const mockRepo = createMockRepository(
      ResultUtils.ok(createMockMovement({ id: 'inv-001', productId: 'prod-001', quantity: 10 }))
    )
  ...
  it('should register saida movement', async () => {
    const mockRepo = createMockRepository(
      ResultUtils.ok(createMockMovement({ id: 'inv-002', productId: 'prod-001', quantity: 5 }))
    )
  ...
  it('should register ajuste movement with description', async () => {
    const mockRepo = createMockRepository(
      ResultUtils.ok(createMockMovement({ id: 'inv-003', productId: 'prod-001', quantity: 15 }))
    )
