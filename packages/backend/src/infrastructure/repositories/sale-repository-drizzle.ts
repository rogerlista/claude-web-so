...
const rowToSaleItem = (row: SaleItemRow): Result<SaleItem, string> => {
	...
	if (!productIdResult.ok) {
		return ResultUtils.err(
			`Invalid ProductId in database: ${productIdResult.error}`,
		);
	}
	/* c8 ignore stop */
	...
	if (!saleItemResult.ok) {
		return ResultUtils.err(
			`Invalid SaleItem in database: ${saleItemResult.error}`,
		);
	}
	/* c8 ignore stop */
...
const rowToSale = (
	...
	if (!customerIdResult.ok) {
		return ResultUtils.err(
			`Invalid CustomerId in database: ${customerIdResult.error}`,
		);
	}
	...
	if (!statusResult.ok) {
		return ResultUtils.err(
			`Invalid SaleStatus in database: ${statusResult.error}`,
		);
	}
...
export const createSaleRepositoryDrizzle = (
	...
	findByCustomerId: async (
		...
		try {
			...
			const filteredItemRows = allItemRows.filter((item) =>
				saleIds.includes(item.saleId),
			);
