 ...
 */
const calculateNetTotal = (grossTotal: number, discount: number, addition: number): number => {
...
export const createSale = (input: CreateSaleInput): Result<Sale, string> => {
  ...
  if (netTotal < 0) {
    return ResultUtils.err('Net total cannot be negative (discount exceeds gross total)')
  }
 ...
 */
export const addItemToSale = (sale: Sale, item: SaleItem): Result<Sale, string> => {
  return createSale({
    id: sale.id,
 ...
 * @returns Result with updated sale or error message
 */
export const removeItemFromSale = (sale: Sale, productId: string): Result<Sale, string> => {
  const updatedItems = sale.items.filter((item) => item.productId !== productId)
...
export const updateItemQuantity = (
  ...
  if (itemIndex === -1) {
    return ResultUtils.err(`Item with product ID ${productId} not found in sale`)
  }
 ...
 * @returns Result with updated sale or error message
 */
export const applyDiscount = (sale: Sale, discount: number): Result<Sale, string> => {
  return createSale({
    id: sale.id,
    customerId: sale.customerId,
    items: sale.items,
 ...
 * @returns Result with updated sale or error message
 */
export const applyAddition = (sale: Sale, addition: number): Result<Sale, string> => {
  return createSale({
    id: sale.id,
    customerId: sale.customerId,
    items: sale.items,
 ...
 * @returns Result with updated sale or error message
 */
export const addPaymentToSale = (sale: Sale, payment: SalePayment): Result<Sale, string> => {
  return createSale({
    id: sale.id,
    customerId: sale.customerId,
    items: sale.items,
