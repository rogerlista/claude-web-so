...
describe('OptionUtils', () => {
  ...
  describe('flatMap', () => {
    it('should chain Options when both are some', () => {
      const option = OptionUtils.some(10)
      const chained = OptionUtils.flatMap(option, (x) => OptionUtils.some(x * 2))
    ...
    it('should propagate none from second Option', () => {
      const option = OptionUtils.some(10)
      const chained = OptionUtils.flatMap(option, (_x) => OptionUtils.none<number>())
      expect(chained.some).toBe(false)
    })

    it('should allow changing value type through flatMap', () => {
      const option = OptionUtils.some(42)
      const chained = OptionUtils.flatMap(option, (x) => OptionUtils.some(`Value: ${x}`))
      expect(chained.some).toBe(true)
      if (chained.some) {
  ...
  describe('toResult', () => {
    ...
    it('should work with different error types', () => {
      const option = OptionUtils.none<string>()
      const result = OptionUtils.toResult(option, { code: 404, message: 'Not found' })
      expect(result.ok).toBe(false)
      if (!result.ok) {
