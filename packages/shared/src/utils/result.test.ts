...
describe('ResultUtils', () => {
  ...
  describe('flatMap', () => {
    ...
    it('should propagate error from second Result', () => {
      const result: Result<number, string> = ResultUtils.ok(10)
      const chained = ResultUtils.flatMap(result, (_x) => ResultUtils.err('second error'))
      expect(chained.ok).toBe(false)
      if (!chained.ok) {
    ...
    it('should allow changing value type through flatMap', () => {
      const result: Result<number, string> = ResultUtils.ok(42)
      const chained = ResultUtils.flatMap(result, (x) => ResultUtils.ok(`Value: ${x}`))
      expect(chained.ok).toBe(true)
      if (chained.ok) {
  ...
  describe('mapErr', () => {
    ...
    it('should change error type through mapping', () => {
      ...
      const mapped = ResultUtils.mapErr(result, (err) => Number.parseInt(err))
      expect(mapped.ok).toBe(false)
      if (!mapped.ok) {
  ...
  describe('all', () => {
    ...
    it('should return err with first error when any Result is err', () => {
      const results = [ResultUtils.ok(1), ResultUtils.err('error 2'), ResultUtils.ok(3)]
      const combined = ResultUtils.all(results)
