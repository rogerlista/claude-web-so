...
export const useSalesStore = defineStore('sales', () => {
  ...
  const totalItems = computed(() => {
    ...
    return currentSale.value.items.reduce((sum, item) => sum + item.quantity, 0)
  })
  ...
  const paidAmount = computed(() => {
    ...
    return currentSale.value.payments.reduce((sum, payment) => sum + payment.amount, 0)
  })
   ...
   * Create a new sale
   */
  const createSale = async (customerId = 'customer-default'): Promise<Sale | null> => {
    loading.value = true
    error.value = null
  ...
  const addItem = async (input: AddItemInput): Promise<boolean> => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/vendas/${currentSale.value.id}/items`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const applyDiscount = async (discount: number): Promise<boolean> => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/vendas/${currentSale.value.id}/discount`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
   ...
   * Add payment to sale
   */
  const addPayment = async (paymentMethodCode: string, amount: number): Promise<boolean> => {
    if (!currentSale.value) {
      error.value = 'Nenhuma venda ativa'
      return false
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/vendas/${currentSale.value.id}/payments`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const finalizeSale = async (): Promise<boolean> => {
    ...
    if (!canFinalize.value) {
      error.value = 'Venda não pode ser finalizada (verifique itens e pagamentos)'
      return false
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/vendas/${currentSale.value.id}/finalize`, {
        method: 'POST',
      })
