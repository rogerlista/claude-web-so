...
describe('Background Sync Manager', () => {
  ...
  describe('saveSyncQueue', () => {
    ...
    it('should throw when localStorage.setItem fails', async () => {
      const mockQueue = [
        { id: '1', url: '/test', method: 'POST', headers: {}, timestamp: Date.now() },
      ]
      ...
      await expect(saveSyncQueue(mockQueue)).rejects.toThrow('QuotaExceededError')
  ...
  describe('getSyncQueueSize', () => {
    ...
    it('should return correct queue size', async () => {
      const mockQueue = [
        { id: '1', url: '/api/test1', method: 'POST', headers: {}, timestamp: Date.now() },
        { id: '2', url: '/api/test2', method: 'PUT', headers: {}, timestamp: Date.now() },
      ]
  ...
  describe('registerBackgroundSync', () => {
    ...
    it('should handle missing service worker registration', async () => {
      vi.spyOn(swManager, 'getServiceWorkerRegistration').mockResolvedValue(null)
  ...
  describe('processSync', () => {
    ...
    it('should do nothing when queue is empty', async () => {
      ...
      expect(consoleDebugSpy).toHaveBeenCalledWith('[Sync] No requests in queue')
  ...
  describe('setupSyncListeners', () => {
    ...
    it('should log when connection is lost', () => {
      ...
      expect(consoleInfoSpy).toHaveBeenCalledWith('[Sync] Connection lost, requests will be queued')
