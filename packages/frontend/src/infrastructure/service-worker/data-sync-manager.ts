		...
		for (const op of batch) {
			try {
				...
				if (op.retryCount >= syncConfig.maxRetries) {
					console.error(
						`[DataSync] Operation ${op.id} failed after ${op.retryCount} retries`,
						op,
					);
					syncConfig.onSyncError?.(
	...
	try {
		...
		if (verified.conflict) {
			...
			if (!verified.remoteData || verified.remoteVersion === undefined) {
				throw new Error(
					"Invalid conflict state: missing remote data or version",
				);
			}
	...
	switch (syncConfig.conflictStrategy) {
		...
		case "last-write-wins": {
			...
			const localTimestamp =
				(conflict.localData.updated_at as number) ?? conflict.timestamp;
		...
		default:
			console.warn(
				"[DataSync] Unknown conflict strategy, falling back to server-wins",
			);
			return conflict.remoteData;
	}
};
